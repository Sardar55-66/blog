...
import { getArticle, getArticles, getLoggendInUser, getRandomAvatar, registerUser } from "../api/get-api-data";
...
export const SignUpForm = () => {
    ...
    // const datas = useSelector(state => state.userSignUp.userInfo)
            ...
            <h2 className="edit-article-title">Create new account</h2>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <label>
                        ...
                        })} className="description" type="password"/>
                    </label>
                        <div style={{height: 40, color: 'red'}}>{errors?.pass && errors?.pass?.message && <p style={{margin: 0, color: 'red'}}>{errors.pass.message}</p>}</div> 

                    {/* чекбокс */}
                    <label className="signup-checkbox">
                        <input className="signup-check" {...register('checkbox', {
